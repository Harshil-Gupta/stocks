...
class UpstoxDataSource:
    ...
    def __init__(
        self, api_key: str, api_secret: str, access_token: Optional[str] = None
    ):
    ...
    def get_access_token(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Upstox API timeout during token request")
            return {"error": "timeout"}
        except requests.exceptions.RequestException as e:
            logger.error(f"Upstox API error during token request: {e}")
            return {"error": str(e)}
    ...
    def get_profile(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Upstox API timeout during profile request")
            return {"error": "timeout"}
        except requests.exceptions.RequestException as e:
            logger.error(f"Upstox API error during profile request: {e}")
            return {"error": str(e)}
    ...
    def get_ltp(self, instrument: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"Upstox LTP timeout for {instrument}")
            return None
        except requests.exceptions.HTTPError as e:
            logger.warning(f"Upstox LTP HTTP error: {e.response.status_code}")
            return None
        except requests.exceptions.RequestException as e:
    ...
    def get_quote(self, instrument: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"Upstox quote error: {response.status_code} - {response.text}"
                )
                return None
    ...
    def get_historical_data(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"Upstox historical error: {response.status_code} - {response.text}"
                )
                return None
        except Exception as e:
...
class UpstoxDataEngine:
    ...
    def __init__(
        ...
        if self.use_upstox:
            ...
            logger.warning(
                "Upstox access token not configured, using yfinance fallback"
            )
    ...
    async def get_price_data(
        ...
        if self.use_upstox:
            ...
            df = self.upstox.get_historical_data(
                symbol, interval="1day", from_date=from_date, to_date=to_date
            )
