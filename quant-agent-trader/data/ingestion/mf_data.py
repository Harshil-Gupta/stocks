...
class MFDataSource:
    ...
    async def get_mf_holdings(self, symbol: str) -> MFHoldingsData:
        ...
        try:
            ...
            if major_holders is not None and not major_holders.empty:
                df = major_holders
                holder_col = df.iloc[:, 0].astype(str).str.lower()
                mf_mask = holder_col.str.contains(
                    "mutual fund|mf ", regex=True, na=False
                )
                if mf_mask.any():
                    mf_row = df[mf_mask].iloc[0]
                    pct = mf_row.iloc[1] if len(mf_row) > 1 else 0
            ...
            if institutional_holders is not None and not institutional_holders.empty:
                ...
                holder_col = df.iloc[:, 0].astype(str).str.lower()
                mf_keywords = ["mutual fund", "mf ", "fund", "insurance"]
                mf_mask = holder_col.str.contains(
                    "|".join(mf_keywords), regex=True, na=False
                )
                mf_related = []
                ...
                if mf_related:
                    ...
                    top_mf_holders = sorted(
                        mf_related, key=lambda x: x["pct"], reverse=True
                    )[:10]
    ...
    def _generate_monthly_trend(
        self, current_pct: float, num_mfs: int
    ) -> List[Dict[str, Any]]:
            ...
            months.append(
                {
                    ...
                    "num_mfs": max(0, num_mfs - random.randint(0, 2))
                    if num_mfs > 0
                    else 0,
                }
            )
    ...
    async def get_multiple_mf_holdings(
        ...
        return {
            ...
            else MFHoldingsData(symbol=symbol)
...
class MFDataEngine:
    ...
    async def get_combined_analysis(self, symbol: str) -> Dict[str, Any]:
    ...
    def _analyze_mf_fii_relationship(
        self, mf_data: MFHoldingsData, fii_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        trend = (
            "accumulating"
            if mf_data.change_in_holding > 0
            else "distributing"
            if mf_data.change_in_holding < 0
            else "stable"
        )
