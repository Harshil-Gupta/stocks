...
async def main():
    ...
    for i, symbol in enumerate(symbols, 1):
        ...
        if result["success"]:
            ...
            score = agg.get("final_score", 0)  # Fixed: use final_score instead of score
            ...
            results.append(
                {
                    "symbol": symbol,
                    "decision": decision.upper(),  # Save as uppercase
    ...
    buy_count = sum(1 for r in results if r.get("decision", "").upper() == "BUY")
    sell_count = sum(1 for r in results if r.get("decision", "").upper() == "SELL")
