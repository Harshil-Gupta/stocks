...
class BacktestEngine:
    ...
    def _prepare_features(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        features = {
            ...
            "volume": float(recent['volume']) if 'volume' in recent else 0.0,
        }
    ...
    def _execute_entry(
        ...
        atr = self._calculate_atr(data, current_date) if len(data) >= 14 else current_price * 0.02
        
        stop_loss = execution_price * (1 - 2 * atr / current_price) if atr > 0 else None
        take_profit = execution_price * (1 + 3 * atr / current_price) if atr > 0 else None
    ...
    def _execute_exit(
        ...
        commission_cost = execution_price * position.quantity * self.config.commission_rate
    ...
    def simulate_trades(
        ...
        for i, date in enumerate(dates[:-1]):
            ...
            for symbol, signal in signals.items():
                ...
                if signal.decision == "buy" and symbol not in self.positions:
                    position_size = min(
                        cash * signal.confidence / 100 * 0.1,
                        cash * 0.1
                    )
                ...
                elif signal.decision == "sell" and symbol in self.positions:
                    position = self.positions[symbol]
                    exit_value = position.quantity * current_price
                    commission = exit_value * self.config.commission_rate
                    
                    pnl = exit_value - (position.entry_price * position.quantity) - commission
                    ...
                    trade = BacktestTrade(
                        ...
                        slippage=0.0,
                        return_percent=pnl / (position.entry_price * position.quantity) * 100,
    ...
    def calculate_metrics(self, equity_curve: List[Dict]) -> BacktestMetrics:
        ...
        cumulative_return = (final_equity - starting_equity) / starting_equity if starting_equity > 0 else 0.0
        ...
        annualized_volatility = np.std(returns) * np.sqrt(self.config.trading_days_per_year) if len(returns) > 0 else 0.0
        
        risk_free_daily = self.config.risk_free_rate / self.config.trading_days_per_year
        excess_returns = returns - risk_free_daily
        
        sharpe_ratio = (
            np.mean(excess_returns) / np.std(excess_returns) * np.sqrt(self.config.trading_days_per_year)
        ...
        sortino_ratio = (
            np.mean(excess_returns) / downside_std * np.sqrt(self.config.trading_days_per_year)
    ...
    def _generate_results(self) -> BacktestResult:
        ...
        if not equity_df.empty:
            ...
            equity_df = pd.DataFrame(columns=['date', 'equity', 'cash', 'positions_value', 'num_positions'])
