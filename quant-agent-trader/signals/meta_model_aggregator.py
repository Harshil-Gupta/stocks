...
class MetaModelAggregator:
    ...
    def __init__(
        self, config: Optional[ModelConfig] = None, model_path: Optional[str] = None
    ):
        """
    ...
    def _train_regime_models(self, dataset: pd.DataFrame, target_column: str) -> None:
        ...
        for regime in self.REGIMES:
            ...
            X = regime_data[self.feature_names].fillna(
                regime_data[self.feature_names].median()
            )
    ...
    def _fallback_aggregation(
        self, signals: List[AgentSignal], regime: str, stock_symbol: str
    ) -> AggregatedSignal:
        ...
        final_score = (avg_score + 1) / 2  # Normalize to 0-100 scale

        # Use new thresholds: >=85 strong_buy, >=60 buy, >=40 hold, >=25 sell, <25 strong_sell
        if final_score >= 85:
            decision = "strong_buy"
        elif final_score >= 60:
            decision = "buy"
    ...
    def _create_empty_signal(self, stock_symbol: str, regime: str) -> AggregatedSignal:
...
class HybridAggregator:
    ...
    def aggregate_signals(
        ...
        confidence = ml_signal.confidence * self.ml_weight + rule_signal.confidence * (
            1 - self.ml_weight
        )
        ...
        return AggregatedSignal(
            ...
            supporting_agents=list(
                set(ml_signal.supporting_agents + rule_signal.supporting_agents)
            ),
            conflicting_agents=list(
                set(ml_signal.conflicting_agents + rule_signal.conflicting_agents)
            ),
            agent_signals=signals,
            regime=regime,
            timestamp=datetime.now(),
