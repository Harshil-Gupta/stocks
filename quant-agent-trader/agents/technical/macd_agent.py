    ...
    def compute_signal(self, features: Dict[str, Any]) -> AgentSignal:
        ...
        try:
            ...
            strong_bullish = (
                macd_above_signal
                and macd_above_zero
                and abs(macd_hist) > self._histogram_strong_threshold
            )
            strong_bearish = (
                macd_below_signal
                and macd_below_zero
            ...
            if strong_bullish:
                if abs(macd_hist) > self._histogram_strong_threshold * 2:
                    signal = "strong_buy"
                else:
                    signal = "buy"
                confidence = min(95.0, 75.0 + abs(macd_hist) * 30)
                numerical_score = min(1.0, abs(macd_hist) * 2)
                reasoning = (
                    f"Strong bullish MACD signal: MACD ({macd:.4f}) is above signal line ({macd_signal:.4f}) "
                    f"and both are above zero. Histogram: {macd_hist:.4f} shows {'very strong' if abs(macd_hist) > self._histogram_strong_threshold * 2 else 'strong'} upward momentum."
                )
                supporting_data = {
                    "macd": macd,
                ...
                reasoning = (
                    ...
                    f"and both are below zero. Histogram: {macd_hist:.4f} shows {'very strong' if abs(macd_hist) > self._histogram_strong_threshold * 2 else 'strong'} downward momentum."
                )
                supporting_data = {
                    "macd": macd,
                    "macd_signal": macd_signal,
            ...
            return self._create_error_signal(
                f"MACD signal computation failed: {str(e)}"
            )
